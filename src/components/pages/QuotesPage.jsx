...
<div className="flex min-h-screen bg-green-50">
    {/* Sidebar */}
    <div
        className="hidden lg:flex lg:flex-col lg:w-64 bg-white border-r border-slate-200">
        {/* Header */}
        ...
        <nav className="flex-1 p-4 space-y-2">
            <div className="space-y-1">
                <button
                    ...
                    <ApperIcon name="Users" className="w-4 h-4" />Contacts
                                </button>
                <button
                    onClick={() => navigate("/companies")}
                    className="w-full flex items-center gap-3 px-3 py-2.5 text-left text-sm font-medium text-slate-700 hover:bg-green-50 rounded-lg transition-colors duration-200">
                    <ApperIcon name="Building" className="w-4 h-4" />Companies
                                </button>
                <button
                    onClick={() => navigate("/deals")}
                    className="w-full flex items-center gap-3 px-3 py-2.5 text-left text-sm font-medium text-slate-700 hover:bg-green-50 rounded-lg transition-colors duration-200">
                    <ApperIcon name="TrendingUp" className="w-4 h-4" />Deals
                                </button>
                <button
                    onClick={() => navigate("/tasks")}
                    className="w-full flex items-center gap-3 px-3 py-2.5 text-left text-sm font-medium text-slate-700 hover:bg-green-50 rounded-lg transition-colors duration-200">
                    <ApperIcon name="CheckSquare" className="w-4 h-4" />Tasks
                                </button>
                <button
                    onClick={() => navigate("/activities")}
                    className="w-full flex items-center gap-3 px-3 py-2.5 text-left text-sm font-medium text-slate-700 hover:bg-green-50 rounded-lg transition-colors duration-200">
                    <ApperIcon name="Activity" className="w-4 h-4" />Activities
                                </button>
                <button
                    ...
                    <ApperIcon name="Receipt" className="w-4 h-4" />Quotes
                                </button>
            </div>
        </nav>
    </div>
    ...
    <div className="flex-1 flex">
        ...
        <div className="flex-1 flex">
            ...
            <div
                ...
                <div
                    ...
                    <Button
                        ...
                        <ApperIcon name="Plus" className="w-4 h-4" />Add Quote
                                    </Button>
                </div>
                {/* Quote List Content */}
                <QuoteList
            ...
            {/* Quote Detail */}
            <motion.div
                className={`flex-1 ${selectedQuote ? "fixed inset-0 lg:relative lg:inset-auto bg-white z-50" : "hidden lg:flex"} flex-col`}
                initial={{
                    opacity: 0
                }}
                animate={{
        ...
        {/* Modals */}
        {showQuoteModal && <QuoteModal quote={editingQuote} onSave={handleSaveQuote} onClose={handleCloseModal} />}
        {showDeleteModal && quoteToDelete && <DeleteQuoteModal
            quote={quoteToDelete}
